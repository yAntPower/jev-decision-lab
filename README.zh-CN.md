...
3. 点击“新建自定义”从空白问题开始：保留自己写的正文，清除未修改的示例正文。也可以应用示例模板。编辑问题、选项及评分等级；高级 JSON 是可选项。
...
自定义评估可提出示例以外的问题。只填写 Choice、Noul 或 Score 中的一种，即可单独运行，不必填好另外两种。Jev 返回选项、是的概率或有序评分，不生成自由格式的长篇报告。
